...
class MaskedBCE: # TO BE TESTED!
    def __init__(self, mask_idxs=None, mask_frac=None):
        try:
    ...
    def __call__(self, output:Tensor, target:Tensor, **kwargs)->Tensor:
        ...
        if self.mask_sel is not None:
            mask_sel = torch.tensor(np.tile(self.mask_sel, (output.shape[0], 1))).float()
        else:
            mask_sel = torch.tensor(np.tile(np.random.binomial(1, self.mask_frac, output.shape[1]), (output.shape[0],1))).float()
        loss = target * torch.log(output) + (1 - target) * torch.log(1 - output)
